...
import image1 from "../../assets/Convocation1.jpg";
import image2 from "../../assets/Convocation2.jpg";
import image3 from "../../assets/Convocation3.jpg";
import image4 from "../../assets/Screenshot 2026-05-03 024620.png";
import image5 from "../../assets/Alumni_meet2026.jpg";
...
const QUOTES = [
  {
    id: 1,
    text: "These photos are more than memories — they're proof that we built something real together. Every reunion feels like coming home.",
    author: "Sarah Chen",
    batch: "Class of 2004",
    avatar:
  ...
  {
    id: 2,
    text: "Flipping through the gallery takes me right back. The laughter, the late nights, the friendships that outlasted everything.",
  ...
  {
    id: 3,
    text: "I uploaded photos from our 10-year reunion and the response was overwhelming. This gallery keeps our story alive.",
    author: "Priya Nair",
    batch: "Class of 2011",
    avatar:
      ...
      <div className={`relative w-full ${h} overflow-hidden bg-gray-100`}>
        {album.coverImage?.url ? (
          ...
          <div
            ...
            style={{
              background: `linear-gradient(135deg,${NAVY}15,${GOLD}20)`,
            }}
          >
        ...
        <div
          className="absolute top-0 left-0 w-[3px] h-full scale-y-0 group-hover:scale-y-100 transition-transform duration-300 origin-bottom"
          style={{ background: GOLD }}
        />
        <div className="absolute bottom-0 left-0 right-0 p-4 translate-y-1 group-hover:translate-y-0 transition-transform duration-300">
          <h3 className="gal-serif font-bold text-white text-lg leading-tight line-clamp-2 mb-2">
            {album.title}
          </h3>
          <div className="flex items-center gap-3 gal-sans text-xs text-white/60">
            ...
            <span className="flex items-center gap-1">
              <Images className="h-3 w-3" style={{ color: GOLD }} />
...
const QuoteCard = ({ quote }) => (
  <div
    className="quote-card bg-white rounded-2xl p-6 border border-gray-100 flex flex-col gap-4"
    style={{ boxShadow: "0 2px 16px rgba(0,0,0,0.05)" }}
  >
    <Quote
      className="h-6 w-6 opacity-25 flex-shrink-0"
      style={{ color: GOLD }}
    />
    <p className="gal-serif text-[15px] italic text-gray-600 leading-relaxed flex-1">
      "{quote.text}"
    </p>
    <div className="flex items-center gap-3 pt-3 border-t border-gray-100">
      ...
      />
      <div>
        <p className="gal-sans text-sm font-semibold text-gray-900">
          {quote.author}
        </p>
          ...
          {loading.photos ? (
            ...
            <>
              <div className="flex-shrink-0 px-6 py-5 border-b border-gray-100">
                <h2 className="gal-serif font-bold text-2xl text-gray-900">
                  {activeAlbum.title}
                </h2>
                  ...
                  {activeAlbum.eventDate && (
                    <span className="flex items-center gap-1.5">
                      <Calendar
                        ...
                        style={{ color: GOLD }}
                      />
                      {new Date(activeAlbum.eventDate).toLocaleDateString(
                        "en-GB",
  ...
  const handleFiles = (newFiles) => {
    ...
    setFiles((prev) =>
      [...prev, ...incoming]
        .filter(
          (f, i, s) =>
            i === s.findIndex((x) => x.name === f.name && x.size === f.size),
        )
        .slice(0, 50),
    );
    setPreviews((prev) =>
      [...prev, ...incoming.map((f) => URL.createObjectURL(f))].slice(0, 50),
    );
  };
  const handleSubmit = () => {
    ...
    >
      <DialogContent className="max-w-xl p-0 gap-0 rounded-2xl border-0 overflow-hidden max-h-[90vh] flex flex-col [&>button]:p-1  [&>button]:rounded-full  [&>button]:cursor-pointer [&>button]:top-4 [&>button]:right-4 [&>button]:z-20 [&>button]:bg-gray-100">
        <div
          ...
          style={{ background: NAVY, padding: "24px 24px 20px" }}
        >
          <div
          ...
          <h2 className="gal-serif font-bold text-xl text-white">
            Share Your Photos
          </h2>
          <p className="gal-sans text-sm text-white/50 mt-1">
            Upload a photo album — admin will review and publish it.
          </p>
        </div>
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4 gal-sans">
          ...
          <div>
            ...
            >
              ...
              <div
                ...
                style={{ background: dragging ? GOLD : "#f5f5f5" }}
              >
                <ImagePlus
                  className={`h-5 w-5 pointer-events-none ${dragging ? "text-white" : "text-gray-400"}`}
                />
              </div>
            ...
            <p className="text-xs text-white p-3 rounded-xl mt-2 bg-green-400">
              Hold <span className="font-medium">Ctrl</span> (or{" "}
              <span className="font-medium">Cmd</span> on Mac) or{" "}
              <span className="font-medium">Shift</span> to select multiple
              files
            </p>
            {previews.length > 0 && (
              <div className="mt-3">
                ...
                <div className="grid grid-cols-5 gap-1.5 max-h-40 overflow-y-auto">
                  {previews.slice(0, 20).map((url, i) => (
                    ...
                    >
                      ...
                      <button
                        onClick={() => {
                          setFiles((p) => p.filter((_, di) => di !== i));
                          setPreviews((p) => p.filter((_, di) => di !== i));
                        }}
          ...
          <div className="flex items-start gap-3 p-3.5 rounded-xl bg-amber-50 border border-amber-100">
            ...
            <p className="text-xs text-amber-700 leading-relaxed">
              Your album will be reviewed before going live. Track status in{" "}
        ...
        <div className="flex-shrink-0 px-6 py-4 border-t border-gray-100 flex items-center justify-end gap-3">
          ...
          <button
            ...
            className="flex items-center gap-2 px-6 py-2.5 rounded-xl text-sm font-semibold transition-all disabled:opacity-50 hover:opacity-90 gal-sans"
...
const MyUploadsDialog = ({ open, onClose }) => {
  ...
  useEffect(() => {
    if (open) dispatch(fetchMyAlbums());
  }, [open]);
  return (
    <Dialog
      open={open}
    ...
    >
      <DialogContent
        className="max-w-lg p-0 gap-0 rounded-2xl border-0 overflow-hidden max-h-[80vh] flex flex-col [&>button]:top-4 [&>button]:right4 [&>button]:z-20 [&>button]:rounded-full 
    [&>button]:p-1 [&>button]:cursor-pointer 
      [&>button]:w9 [&>button]:h9 [&>button]:bg-gray-100"
      >
        <div className="flex-shrink-0 px-6 pt-6 pb-4 border-b border-gray-100">
          <h2 className="gal-serif font-bold text-xl text-gray-900">
            My Uploads
          </h2>
          <p className="gal-sans text-sm text-gray-400 mt-1">
            Track your submitted photo albums
          </p>
        </div>
          ...
          {loading.mine ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <p className="text-sm text-gray-400">
                You haven't submitted any albums yet.
              </p>
            </div>
          ) : (
            <div className="space-y-3">
              {myAlbums.map((album) => {
                ...
                return (
                  <div
                    key={album._id}
                    className="flex items-center gap-4 p-3.5 rounded-xl border border-gray-100 bg-white"
                  >
                    <div className="w-14 h-14 rounded-xl overflow-hidden flex-shrink-0 bg-gray-100">
                      {album.coverImage?.url ? (
                        ...
                        <div className="w-full h-full flex items-center justify-center">
                          <Images className="h-5 w-5 text-gray-300" />
                        </div>
                      )}
                    </div>
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-400">
                        {album.photoCount} photos · {fmtDate(album.createdAt)}
                      </p>
                    ...
                    <span
                      className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wide ${sc.color}`}
                    >
...
const UserGallery = () => {
  ...
  const cols = [[], [], []];
  albums.forEach((a, i) =>
    cols[i % 3].push({ album: a, rowIndex: Math.floor(i / 3) }),
  );
  ...
  return (
    <>
      ...
      <div
        className="min-h-screen gal-sans bg-white "
        style={{ background: "#" }}
      >
        {/* ══════════════════════════════════════════
        ...
        >
          ...
          <div
            className="relative z-10 max-w-7xl mx-auto px-6 sm:px-10 flex flex-col lg:flex-row items-center gap-12 py-8 md:py-10"
            style={{ minHeight: 520 }}
          >
            {/* LEFT — Text block */}
            <div className="flex-1 max-w-xl">
              ...
              >
                Browse curated albums from graduation ceremonies, reunions, and
                campus milestones spanning decades of our global alumni
                community.
              </p>
              ...
              <div className="flex items-center gap-3 md:mt-10 flex-wrap hero-text-delay2">
                <button
                  ...
                  style={{
                    background: NAVY,
                    color: "#fff",
                    boxShadow: `0 8px 28px ${NAVY}40`,
                  }}
                >
                  <Upload className="h-4 w-4 flex-shrink-0" /> Upload &amp;
                  Share Photos
                </button>
                <button
            ...
            <div className="flex-shrink-0 relative hidden lg:block hero-text-delay2 ml-auto  w-[550px] h-[430px]">
              ...
              <div
                className="card-f1 absolute overflow-hidden rounded-2xl z-10 border-[5px] border-white top-[68px] left-[10px] w-[280px] h-[340px]"
                style={{
                  boxShadow:
              ...
              >
                <img
                  src={image4}
                  alt=""
                  className="w-full object-cover h-[84%]"
                />
                <div className="px-3 pt-1.5 pb-2 bg-white">
                  <p className="gal-sans text-[10px] font-bold uppercase tracking-wider text-gray-400">
                    Farewell 2026
                  </p>
                </div>
              </div>
              ...
              <div
                className="card-f2 absolute overflow-hidden rounded-2xl z-30 border-[5px] border-white top-2 left-[148px] w-[310px] h-[372px]"
                style={{
                  boxShadow:
                    "0 30px 72px rgba(0,0,0,0.24),0 8px 20px rgba(0,0,0,0.10)",
                }}
              >
                <img
                  src={image3}
              ...
              <div
                className="card-f3 absolute overflow-hidden rounded-2xl z-20 border-[5px] border-white top-[88px] left-[298px] w-[275px] h-[330px]"
                style={{
                  boxShadow:
                    "0 14px 40px rgba(0,0,0,0.14),0 4px 10px rgba(0,0,0,0.07)",
                }}
              >
                <img
                  src={image5}
        ...
        {/* ══ STICKY SEARCH ══ */}
        <div
          className="sticky top-0 z-30 border-b shadow-sm bg-gray-100"
          style={{ backdropFilter: "blur(12px)", borderColor: `${NAVY}12` }}
        >
        ...
        <div
          ref={listRef}
          className="max-w-7xl bg-white mx-auto px-4 sm:px-6 py-10 sm:py-12"
        >
          {!loading.albums && albums.length === 0 && (
            <div className="flex flex-col items-center justify-center py-24 text-center">
              ...
              <h3 className="gal-serif text-xl font-bold text-gray-700 mb-2">
                No albums yet
              </h3>
              <p className="text-sm text-gray-400 max-w-xs gal-sans">
                {searchText
                  ? `No albums found for "${searchText}".`
                  : "Be the first to share your photos!"}
              </p>
              {searchText ? (
                <button
                  onClick={clearSearch}
                  className="mt-4 px-5 py-2 rounded-xl text-sm font-semibold text-white transition hover:opacity-90 gal-sans"
                ...
                <button
                  onClick={() => setUploadOpen(true)}
                  className="mt-4 flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold transition hover:opacity-90 gal-sans"
                  style={{ background: GOLD, color: NAVY }}
                >
          ...
          {albums.length > 0 && (
            <div
              className={`transition-opacity duration-300 ${loading.albums ? "opacity-40 pointer-events-none" : "opacity-100"}`}
            >
